...
then
        ...
        then
            ...
            strResult="false"
            strReason="System uses pam_tally2.so, but the file /var/log/tallylog does not exist."
        fi
else
        strResult="true"
        strReason="File /var/log/tallylog does not exist. But, it is not required if pam_tally2.so is not used."
fi
