...
if __name__ == '__main__':
    ...
    target_file = "/etc/csh.login"
    comp_id = "Linux.3-2-f"
    outstr = ""
    str_compliant = ""
    str_reason = ""
    # Check if /etc/csh.login exists.
    if not os.path.exists(prefix+target_file):
        str_compliant = "false"
        str_reason = "File "+target_file+" must exist."
    else:
        
        # If exists, it must have a line that invokes IBMsinit.csh
        found="false"
        after_text=[]
        with open(prefix+target_file, 'r') as fp:
            for line in fp:
                if "source /etc/profile.d/IBMsinit.csh" in line and line[0]=='.':
                    found="true"
                if found=="true":
        ...
        if found=="false":
            str_compliant = "false"
            str_reason = "File "+target_file+" must call IBMsinit.csh."
        else:

            # And, umaks should not be called after IBMsinit.csh is invoked.
            ...
            if violation_found=="true":
                str_compliant = "false"
                str_reason = "Umask is set/reset after IBMsinit.csh is invoked."
            else:
                str_compliant = "true"
                str_reason = "File "+target_file+" calls IBMsinit.csh and umaks is not set/reset."
