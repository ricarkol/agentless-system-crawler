...
feature_table_column_header_mappings = {
  'package': {
   ...
   'file' : {
     'table_columns': ['Path', 'Size', 'User id', 'Group id', 'Permissions', 'Type'],
     'feature_attributes': ['path', 'size', 'uid', 'gid', 'mode', 'type']
    }
}

def feature_att_value_filter(feature_type, feature):
    if feature_type == 'file':
        feature['mode'] = oct(feature['mode'])
...
def get_feature_table_row(feature_type, feature):
    feature_att_value_filter(feature_type, feature)
