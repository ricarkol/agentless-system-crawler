...
PROC_ID=$2
CONTAINER_NAME=${CONFIG_PARSER_CONT}_${PROC_ID}
...
case $1 in
    start)
        ...
        if [ ! -z "$REGISTRY" ]; then
            set -x
            docker pull $REGISTRY/$CONFIG_PARSER_IMG:$IMAGE_TAG 2>&1 > /dev/null
            docker tag -f $REGISTRY/$CONFIG_PARSER_IMG:$IMAGE_TAG $CONFIG_PARSER_IMG 
            set +x
        fi
