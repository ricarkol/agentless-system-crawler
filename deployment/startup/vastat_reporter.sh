...
PROC_ID=$2
CONTAINER_NAME=${VASTAT_REPORTER_CONT}_$PROC_ID
...
case $1 in
    start)
        ...
        if [ ! -z "$REGISTRY" ]; then
            set -x
            docker pull $REGISTRY/$VASTAT_REPORTER_IMG:$IMAGE_TAG 2>&1 > /dev/null
            docker tag -f $REGISTRY/$VASTAT_REPORTER_IMG:$IMAGE_TAG $VASTAT_REPORTER_IMG 
            set +x
        fi
        # start vastat reporter 
        set -x		
