...
PACKAGE=vacrawler_0.8-va-crawler_amd64.deb
...
then
	...
	#this is a XXX hack. Forgot to change alchemy-crawler to vacrawler in the pre-removal fpm script
        ...
        $SSH ${SSH_USER}@$host HOST=$host /usr/bin/sudo /usr/bin/dpkg --purge vacrawler || true
	...
	$SSH ${SSH_USER}@$host HOST=$host /usr/bin/sudo /usr/bin/dpkg -i $PACKAGE || echo "Could not install va-crawler package"
