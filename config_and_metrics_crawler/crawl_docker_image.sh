...
docker run -d --entrypoint="/bin/sleep.crawler" -v $BINARIES/sleep:/bin/sleep.crawler \
...
then
        send_notification "start"
        send_empty_frame
        send_notification "error" "Could not instantiate the image, sent an empty frame."
        docker rm -f ${CONTAINER_NAME}
        # Return 0 so regcrawler doesn't retry.
	exit 0
fi
