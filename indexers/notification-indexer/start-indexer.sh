...
sed -i s"/<TYPE>/$DOC_TYPE/" /etc/indexer.conf
sed -i s"/<GROUP_ID>/$GROUP_ID/" /etc/indexer.conf
sed -i s"/<KAFKA_TOPIC>/$KAFKA_TOPIC/" /etc/indexer.conf
sed -i s"/<KAFKA_TOPIC>/$KAFKA_TOPIC/" /opt/index-template.json
