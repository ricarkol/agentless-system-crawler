...
def get_metadata_json(instance_identifier, type):
    ...
    if type == "docker":
        ...
        if os.path.exists("/openstack/nova/metadata/"+ instance_identifier + ".json"):
             logger.debug("Found Nova metadata for %s" % instance_identifier) 
             json_data = open("/openstack/nova/metadata/"+ instance_identifier + ".json").read()
             found = True
        elif os.path.exists("/var/lib/nova/metadata/"+ instance_identifier + ".json"):
             logger.debug("Found Nova metadata for %s" % instance_identifier) 
             json_data = open("/var/lib/nova/metadata/"+ instance_identifier + ".json").read()
             found = True
    
    if type == "kvm":
        # instance_identifier is the libvirt domain-name
        config_drive_paths = lookup_config_drive_path(instance_identifier)
        mountdir = None
        # iterate through each potential config-drive to find metadata until one is found
        for config_drive_path in config_drive_paths:
            if not found:
                try:
                    mountdir = subprocess.Popen(['/bin/mktemp', '-d'], stdout=subprocess.PIPE).communicate()[0].strip()
                    subprocess.Popen(["mount", "-o", "loop", config_drive_path, mountdir], stdout=subprocess.PIPE).communicate()
                    if os.path.exists(mountdir + "/openstack/latest/meta_data.json"):
                        json_data = open(mountdir + "/openstack/latest/meta_data.json").read()
                        found = True
                        logger.debug("Found Nova metadata for %s" % instance_identifier)
                    subprocess.Popen(["umount", mountdir], stdout=subprocess.PIPE).communicate()
                    subprocess.Popen(["rmdir", mountdir], stdout=subprocess.PIPE).communicate()
...
def get_namespace(instance_identifier, type):
    ...
    try:
        ...
        logger.warning("VM %s does not appear to have a valid metadata property "
                       "file." % (instance_identifier))
        return None
    namespace = ''
...
"""
Generate a directory path where to link the container log files
in the host. Does not include the base directory.
"""
