...
do
	...
	docker run -d -e LOG_LOCATIONS=/var/log/messages --name test_crawl_cpu_many_containers_$i \
		ubuntu bash -c "echo $MSG >> /var/log/messages; echo $MSG; sleep 5; echo $MSG >> /var/log/messages; echo $MSG; sleep 6000 " 2> /dev/null > /dev/null
...
do
	MSG=${CONTAINER_ID[$i]}
	R=`grep -c $MSG /var/log/crawler_container_logs/${SPACE_ID[$i]}/0000/${CONTAINER_ID[$i]}/var/log/messages`
