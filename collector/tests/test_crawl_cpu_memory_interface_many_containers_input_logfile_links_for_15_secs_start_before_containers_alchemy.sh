...
python2.7 ${CRAWLER_CODE} --crawlmode OUTCONTAINER \
	--features=cpu,memory,interface --numprocesses 4 \
	--frequency 1 --linkContainerLogFiles --environment alchemy --format graphite \
	--url file:///tmp/alchemy_all_test > /dev/null 2> /dev/null &
PID=$!

# start 5 seconds before the containers are started
sleep 5
...
# now sleep 10 seconds and kill the crawler
sleep 10
exec 2> /dev/null
