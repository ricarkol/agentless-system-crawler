...
# percentile() function copied from http://code.activestate.com/recipes/511478-finding-the-percentile-of-the-values/
def percentile(N, percent, key=lambda x:x):
    ...
    if not N:
        return -1
    # SORT N, this was not in teh original code
    N.sort()
    k = (len(N)-1) * percent
    f = math.floor(k)
    c = math.ceil(k)
...
def verbose_ping(dest_addr, timeout = 2, count = 4, psize = 64):
    ...
    for i in xrange(count):
        ...
        if delay  ==  None:
            ...
            print "get ping in %0.4fus" % delay
    print


def quiet_ping(dest_addr, timeout = 2, duration = 1, psize = 64, sleep = 0.5):
    """
    Send pings for 'duration' seconds with `psize' size to `dest_addr' with
    the given `timeout' and display the result.
    Returns `percent' lost packages, `p95' round trip time
    ...
    percent_lost = 100 - (len(plist) * 100 / i)

    # Find max and avg round trip time
    if plist:
        artt = sum(plist) / len(plist)
        p95 = percentile(plist, 0.95)
        p99 = percentile(plist, 0.99)

    return percent_lost, artt, p95, p99

# tester
if __name__ == '__main__':
    #verbose_ping("heise.de")
    verbose_ping("google.com")
    print quiet_ping("google.com")
    print quiet_ping("logs.opvis.bluemix.net", timeout=2, duration=60, sleep=1)
