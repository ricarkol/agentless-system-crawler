...
echo "FROM scratch" > Dockerbuild.test
echo "ADD test_crawl_docker_scratch_image_to_file.sh ." >> Dockerbuild.test

# Temporarily create an image that looks like a bluemix image
docker build -f Dockerbuild.test -t $IMAGE .  > /dev/null 2> /dev/null
...
# We added this file to the image, let's check it's there
COUNT5=`grep '/test_crawl_docker_scratch_image_to_file.sh' $FRAME.* | wc -l`
